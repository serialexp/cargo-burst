...
- **Output streams live — DO NOT pipe through `| tail -N`.** stdout/stderr from cargo come back over the SSH session line-by-line. `tail` buffers everything until EOF and only emits its window at the end, which means a long compile or test run looks like a total silence followed by a wall of text (or, if the bash timeout fires first, *nothing at all* — tail discards its buffer when killed). Just run `cargo burst test`/`build`/etc. without piping through tail; the output is already terse (cargo's normal Compiling lines + nextest's PASS lines), and you want to see it stream so you can tell progress from a real hang. If you genuinely need to bound output size, redirect to a file and `tail -f` from another shell.
- **Diagnosing a stuck run.** If a `cargo burst` command appears to produce no output for minutes, run `cargo burst status` in a separate shell *before* concluding it's hung. The top-processes table will show whether `rustc`/`clippy`/`cargo nextest` are actively burning CPU on the remote (in which case the local silence is `tail` buffering, not a hang) or whether everything is idle (real hang — grab the PIDs to kill, or `cargo burst down`).
